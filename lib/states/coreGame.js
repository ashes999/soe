(function() {
  ...
  window.CoreGame = (function() {
    ...
    CoreGame.prototype.create = function() {
      ...
      console.debug("Showing map " + this.currentMap.x + ", " + this.currentMap.y);
    ...
    CoreGame.prototype.update = function() {
      ...
      return this.game.physics.arcade.overlap(this.player, this.transitionTiles, function(player, tile) {
        ...
        if (t.direction === 'right') playerX += TILE_SIZE.width;
        if (t.direction === 'left') playerX -= TILE_SIZE.width;
        playerY = t.destY * TILE_SIZE.height;
        if (t.direction === 'down') playerY += TILE_SIZE.height;
        if (t.direction === 'up') playerY -= TILE_SIZE.height;
    ...
    CoreGame.prototype._showMap = function(map) {
      var t, tile, tiles, x, y, _i, _len, _ref, _ref2, _ref3;
      this.currentMap = map;
      console.debug("Map is " + map.width + "x" + map.height + " tiles");
      if (this.transitionTiles != null) this.transitionTiles.destroy;
      if (this.collideTiles != null) this.collideTiles.destroy;
      if (this.tiles != null) this.tiles.destroy;
      this.collideTiles = this.game.add.group();
      this.collideTiles.enableBody = true;
      this.tiles = this.game.add.group();
      this.transitionTiles = this.game.add.group();
      for (y = 0, _ref = map.height; 0 <= _ref ? y < _ref : y > _ref; 0 <= _ref ? y++ : y--) {
        for (x = 0, _ref2 = map.width; 0 <= _ref2 ? x < _ref2 : x > _ref2; 0 <= _ref2 ? x++ : x--) {
      ...
      for (_i = 0, _len = _ref3.length; _i < _len; _i++) {
        ...
        if (tiles.total === 0) throw "Found no tiles at " + t.x + ", " + t.y;
        tile = tiles.first;
        this.game.physics.enable(tile, Phaser.Physics.ARCADE);
        this.transitionTiles.add(tile);
        tile.body.immovable = true;
      }
      return console.debug("Total tiles: " + (this.collideTiles.total + this.transitionTiles.total + this.tiles.total));
    };
